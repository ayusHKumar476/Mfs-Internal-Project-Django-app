...
def get_available_websites(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"message": "success", "data":store_details}, status=200)

        except Exception as e:
            return JsonResponse(
                {"message": f"Something went wrong: {str(e)}"}, status=400
            )

    if request.method == "GET":
        try:
            data_list = get_scrapped_company()
            return JsonResponse({"message": "success", "data": data_list}, status=200)
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                return HttpResponse(
                    "Invalid email header. Please check your email content."
                )
